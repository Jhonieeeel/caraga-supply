        ...
        <div @class([
            ...
            @isset($filter['quantity'])
                <div class="{{ $personalize['filter.quantity'] }}">
                    <x-dynamic-component :component="TallStackUi::prefix('select.styled')" :label="data_get($placeholders, 'quantity')" :options="$quantity"
                        wire:model.live="{{ $filter['quantity'] }}" required invalidate />
                </div>
            @endisset
            @isset($filter['search'])
                <div class="{{ $personalize['filter.search'] }}">
                    <x-dynamic-component :component="TallStackUi::prefix('input')" :icon="TallStackUi::icon('magnifying-glass')"
                        wire:model.live.debounce.500ms="{{ $filter['search'] }}" :placeholder="data_get($placeholders, 'search')" type="search"
                        invalidate />
                </div>
            @endisset
        </div>
    ...
    <div class="{{ $personalize['wrapper'] }}">
        <div class="{{ $personalize['table.wrapper'] }}">
            <table class="{{ $personalize['table.base'] }}"
                ...
                @if ($livewire && $loading)
                    <x-tallstack-ui::icon.generic.loading class="{{ $personalize['loading.icon'] }}"
                        wire:loading="{{ $target }}" />
                @endif
                @if (!$headerless)
                    <thead @class([
                        'uppercase',
                        $personalize['table.thead.normal'] => !$striped,
                        $personalize['table.thead.striped'] => $striped,
                    ])>
                        <tr>
                            @if ($selectable)
                                <th @class(['w-6', $personalize['table.th']]) wire:key="checkall-{{ implode(',', $ids()) }}">
                                    <x-dynamic-component :component="TallStackUi::prefix('checkbox')" x-ref="checkbox"
                                        x-on:click="all($el.checked, {{ \Illuminate\Support\Js::from($ids()) }})"
                                        dusk="tallstackui_table_select_all" sm />
                                </th>
                            @endif
                            @foreach ($headers as $header)
                                <th scope="col" class="{{ $personalize['table.th'] }}">
                                    <a
                                        @if ($livewire && $sortable($header)) class="inline-flex cursor-pointer truncate"
                                            wire:click="$set('sort', {column: '{{ $head($header)['column'] }}', direction: '{{ $head($header)['direction'] }}' })" @endif>
                                        ...
                                        @if ($livewire && $sortable($header))
                                            <x-dynamic-component :component="TallStackUi::prefix('icon')" :icon="TallStackUi::icon($sorted($header) ? ($head($header)['direction'] === 'desc' ? 'chevron-up' : 'chevron-down') : 'chevron-up-down')" internal
                ...
                <tbody class="{{ $personalize['table.tbody'] }}">
                    ...
                    @else
                        @forelse ($rows as $key => $value)
                            ...
                            @endphp
                            <tr @class([
                                $personalize['table.tr'],
                                'bg-gray-50 dark:bg-dark-800/50' => $striped && $loop->index % 2 === 0,
                            ])
                                @if ($livewire) wire:key="{{ $id }}" @endif>
                                @if ($selectable)
                                    <td class="{{ $personalize['table.td'] }}">
                                        <x-dynamic-component :component="TallStackUi::prefix('checkbox')" id="checkbox-{{ $key }}"
                                            :attributes="$modifier()" value="{{ data_get($value, $selectableProperty) }}"
                                ...
                                @foreach ($headers as $header)
                                    ...
                                    @endphp
                                    @isset(${'column_' . $row})
                                        <td @if ($clickable) x-on:click.prevent="redirect(@js($url), @js($blank))" @endif
