<div>
    ...
    <div>
        <x-table :$headers id="procurement"
            :rows='$this->rows'
            filter
            :quantity="[2,5,10]" >
            ...
            @interact('column_action', $procurement)
                   <div class="sm:flex items-center gap-x-4">
                        ...
                        <x-button.circle color="teal" icon="document-text" wire:click="edit({{ $procurement->id }})"/>
                   </div>
            @endinteract
        </x-table>
    </div>
    ...
    <x-modal title="Annual Procurment Plan" id="edit-entry" size="4xl">
        ...
        <form class="grid grid-cols-2 gap-4 w-full" wire:submit.prevent='onUpdate'>
            ...
            <x-select.styled label="Mode of Procurement *" wire:model="annualForm.mode_of_procurement" :options="[
            ...
            <x-select.styled
                label="Select Year *"
                :options="$years"
                wire:model="annualForm.app_year"
            />
            <div class="col-span-2">
