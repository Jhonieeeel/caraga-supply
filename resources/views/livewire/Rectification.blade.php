    ...
    <x-tab selected="Rectification">
                        ...
                        <template x-for="(row, index) in rows" :key="index">
                            <div class="flex flex-col sm:flex-row items-end gap-4 mt-3">
                                ...
                                <x-number min="0" max="59" label="Minutes:" class="w-full sm:w-24"
                                    x-model="row.minutes" />
                                <x-select.native label="Action:" :options="[
                                    'In',
                                    'Out',
                                    'OB',
                                    'OT',
                                ...
                                <x-button outline icon="minus" red x-show="rows.length > 1"
                                    x-on:click="removeRow(index)" color="red">
                                </x-button>
                            </div>
                        </template>
                            ...
                            <table class="min-w-full border-collapse">
                                ...
                                <tbody>
                                    @foreach ($rows as $index => $row)
                                        <tr
                                            class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-100' }} hover:bg-gray-200">
        ...
        <x-tab.items tab="Upload DTR">
            <div class="space-y-4 p-4">
                ...
                <div class="flex">
                    <x-button text="Upload" color="emerald" wire:click="uploadDTR" class="p-2" />
                    <x-link href="http://10.20.2.21:8080/zkteco_project/" color="slate" light
                        text="Upload via Network" icon="link" class="p-2 ml-auto" />

                </div>
            </div>
        </x-tab.items>
        <x-tab.items tab="Manage Signatory">
            <livewire:signatory></livewire:signatory>
        </x-tab.items>
    </x-tab>
</div>
